...
class ReadRFID extends PureComponent {
  ...
  componentDidMount () {
    if (this.state.type === 'borrow' && this.props.bookData.length > 0) {
      this.setState({ listData: this.props.bookData })
      this.props.changeStep()
    }
    else if (this.state.type === 'invert' && this.props.invertData.length > 0) {
      this.setState({ listData: this.props.invertData })
  ...
  commitBook(){
    if (this.state.type === 'invert') {
       this.props.commitInvertBook(this.state.listData)
    } else {
  ...
  render() {
    const { borrow,invert } = this.props
    if (this.state.commit) return <Redirect to={`/index/${this.state.type}/checkBook/${this.state.type}`} /> 
    return (
            ...
            renderItem={(item, index) => (
              <List.Item
                ...
                actions={[
                  ...
                  >
                    {this.type === 'invert' ? '不归还这本书' : '不借阅这本书'}
                  </Button>,
                ]}
...
const mapState = (state) => ({
  ...
  invertData:state.getIn(['invert','bookData'])
})
...
const mapDispatch = (dispatch) => ({
  ...
  commitInvertBook(listData) {
    dispatch(invertActionCreators.commitBorrowedBooks(listData))
  },
  changeStep() {
    dispatch(actionCreators.changeStep(0))
  },
  changeInvertStep() {
    dispatch(invertActionCreators.changeStep(0))
  },
})
